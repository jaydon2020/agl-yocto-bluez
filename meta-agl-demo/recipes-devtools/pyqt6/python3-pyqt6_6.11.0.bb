...
SRC_URI += "file://0001-Add-preliminary-Qt6-6.12-compatibility.patch"
...
do_configure() {
    ...
    # Replace host Python include path with Yocto sysroot path to fix:
    # cc1plus: error: include location "/usr/include/python3.14"
    # is unsafe for cross-compilation [-Werror=poison-system-directories]
    find ${B} -name "*.pro" | while read f; do
        sed -i "/^INCLUDEPATH/ s|/usr/include/python${PYTHON_BASEVERSION}|${RECIPE_SYSROOT}${includedir}/python${PYTHON_BASEVERSION}|g" "$f"
    done
